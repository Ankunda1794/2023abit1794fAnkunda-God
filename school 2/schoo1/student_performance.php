...
// Restrict to Director of Studies
...
// Fetch performance of selected student
$records = [];
if ($student_id) {
    $q = $conn->prepare("
        SELECT subject, U1, U2, U3, U4, MT, EOT, total, grade
...
<!DOCTYPE html>
<html><head>
    <title>Performance & Report Card</title>
    <style>
...
<h2>Class Performance & Report Card</h2>
...
<form method="get">
    ...
    <select name="term" onchange="this.form.submit()">
        <?php foreach($terms as $t): ?>
          <option <?= $t==$term?'selected':'' ?>>Term <?= $t ?></option>
        <?php endforeach ?>
    </select>
    <select name="student_id" onchange="this.form.submit()">
      <option value="">-- Select Student --</option>
      <?php foreach($students as $id=>$n): ?>
        <option value="<?=$id?>" <?= $id==$student_id?'selected':'' ?>>
          <?=$n?> (<?=$id?>)
        </option>
      <?php endforeach?>
    </select>
...
<?php if ($student_id): ?>
  <?php if ($records): ?>
    <table>
      <tr>
        <th>Subject</th><th>U1</th><th>U2</th><th>U3</th><th>U4</th>
        <th>MT</th><th>EOT</th><th>Total</th><th>Grade</th><th>Action</th>
      </tr>
  ...
  <?php endif?>
<?php endif?>
<?php if ($student_id && $class && $term): ?>
    <div style="text-align:center; margin-top:20px;">
        <a href="generate_report.php?student_id=<?=urlencode($student_id)?>&class=<?=urlencode($class)?>&term=<?=urlencode($term)?>"
